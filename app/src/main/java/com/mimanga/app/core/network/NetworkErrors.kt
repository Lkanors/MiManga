...
/**
 * Отказ по возрасту: тайтл 18+, а возраст не подтверждён.
 *
 * Решает это сервер — он отвечает 403 и на карточку, и на главы, и на
 * страницы. Приложению остаётся отличить такой отказ от обычной сетевой
 * ошибки: при сетевой показывается то, что уже загружено, и человек
 * продолжает читать, а здесь показывать нечего.
 */
fun Throwable.isAgeBlocked(): Boolean = (this as? ServerException)?.status == 403
